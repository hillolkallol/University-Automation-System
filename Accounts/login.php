...
else
{
	if(isset($_POST['username']) && isset($_POST['password']))
	{
		...
		{
			$query = "SELECT * FROM users WHERE username='".mysql_real_escape_string($username)."' AND password='".mysql_real_escape_string($password_hash)."'";
			if($query_run = mysql_query($query))
			{
				if(mysql_num_rows($query_run)==1)
				{
					$sessionUsername = mysql_result($query_run, 0, 'username');
					...
					if($query_run = mysql_query($query))
					{
						if(mysql_num_rows($query_run)==1)
						{
							$accessCategory = mysql_result($query_run, 0, 'category');
							$_SESSION['accessCategory'] = $accessCategory;
		...
		{
			echo '<script type="text/javascript">';
			echo 'alert("You must supply a Username and Password!")';
			echo '</script>';
		}
	}
}
...
if(isset($_POST['recover']))
{
   ...
   if($query_run = mysql_query($query))
   {
      ...
      $link="http://".$_SERVER['SERVER_NAME']."/Automation/Accounts/admin_password_recovery.php?user=".$username."&hash=".$unique_hash;
      echo $link;
               ...
               <div class="breadcumb_area">
                  <div class="container">
                        <div class="row">
                           ...
                           <div class="col-md-10">
                              <div style="font-size:35px; padding-top:50px;">Welcome To LU Accounts Automation System</div>
                           </div>
                        </div>
                  </div>
               </div>
...
<!---------------------------------form Starts---------------------------------------------->
                              <form action="<?php echo $current_file;?>" method="POST">
                                 <div class="form-group input-group">
                                    ...
                                    <input type="text" name="username" placeholder="Your User Name" class="form-control input" required="required">
                                 </div>
                                 <div class="form-group input-group">
                                    <span class="input-group-addon" id="basic-addon1"><i class="fa fa-key"></i></span><input type="password" name="password" placeholder="Your User Password" class="form-control input" required="required">
                                 </div>
         ...
         <div class="container">
            <div class="row">
               <div class="col-md-12">
                  <div class="copy">
                     <p><strong>Design &amp; Developed By:</strong> CSE 26th Batch.Leading University</p>
                  </div>
               </div>
            </div>
         </div>
                               ...
                               <!-- Modal -->
                                 ...
                                 <form action="<?php echo $current_file;?>" method="POST" id="std_register" name="std_register" enctype="multipart/form-data">
                                             <div class="modal-header">
                                                ...
                                                <h4 class="modal-title" id="myModalLabel">Enter your username</h4>
                                             </div>
