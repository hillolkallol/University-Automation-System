<?php
	...
	if(loggedin())
	{
		if(isset($_POST['studentid']) && isset($_POST['sem'])&& isset($_POST['year']))
		{
					...
					<div class="col-md-12">
						<div id="printthis">
							...
							<body>
								...
								<!----------------------------------------------->
								<h4><b>Performance Report For <?php echo $semestername;?> Semester</b></h4>
								<style>
									#border{}
									#border table{
									...
									<div id="border" >
										<table style="width:100%">
											...
											<?php
												...
												//Finding Result
												$grade=null; $marks=null;	$creditearn=0;	$point=0;	$gradepoint=0;	$semestergradepoint=0; $found=0;
												$result = mysql_query("SELECT * FROM student_result");
												while($row2 = mysql_fetch_array($result)) 
												{
													if($row2['s_id']==$studentid  || $row2['s_id']==$old)
													{	
														if($row2['session']==$semester && $row2['year']==$year)
														{
															$found=1;
												...
												//Old id total credit
												$totalcredit3=0;$totalgradepoint3=0;
												...
												<div class="table-responsive" >	
													<table style="width:100%">
															<?php if($found==1 && $creditearn>0)
																{?>
																	<tr>
																		<td>Semester GPA = <?php $valu=round(($semestergradepoint /$creditearn),2);
																		echo number_format((float)$valu, 2, '.', '');?>
																		</td>
																		<td><div id="space1">
																		Credit Earned =<?php $valu1= round($creditearn,2);
																			echo number_format((float)$valu1, 2, '.', ''); ?></div>
																		</td>
																		<td><div id="space1"> <?php  $valu2= round($semestergradepoint,2);
																			echo number_format((float)$valu2, 2, '.', ''); ?></div>
																		</td>
																	</tr>
															<?php } else {?>
																<tr>
																	<td><h4>No Result Found</h4></td>
																</tr>
															<?php }?>
														</table><br><br>
													</div>
