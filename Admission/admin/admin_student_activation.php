...
$query="select * from tbl_student_info where std_auto_id='$id'";
	...
	if(isset($_POST['add_std_submit']))
{
		...
		$std_total_gpa= htmlentities(isset($_POST['txt_std_result'])?$_POST['txt_std_result']:null);
		...
		$query  = "update tbl_student_info set std_dept=$dept,
					...
					std_total_result=$std_total_gpa,std_active=$std_active,
					std_batch=$std_batch,std_semester=1,std_waiver=$std_waiver,std_math='$math',
					std_physics='$physics',std_id=$std_id,std_password='$std_pass' where std_auto_id='$id'";
		...
		if($result)
		{
				...
				echo '</script>';
				$rimon="select * from tbl_student_info where std_auto_id='$id'";
				$resr=mysql_query($rimon);
				$rowr=mysql_fetch_array($resr);
				$idd=$rowr['std_id'];
				$kd="update student_trans set studentId=$idd where studentId='$id'";
				$rk=mysql_query($kd);
				header("location:admin.php");
		}
		else
